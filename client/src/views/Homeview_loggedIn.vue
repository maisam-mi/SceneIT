<template>
  <main id="body">
    <section class="top-picks-section2">
      <div class="popular-header">
        <h2>Recommended</h2>
      </div>
      <div v-show="showMovies" class="scroll-container" id="topPicksScroll">
        <div class="movie-card" v-for="movie in recommendedMovies" :key="movie">
    ...
    <section class="top-picks-section">
      <div class="popular-header">
        ...
        <div class="toggle-switch">
          <button
            :class="['toggle-btn', activeTab === 'movie' ? 'active' : '']"
            @click="setActive('movie')"
          >
            Movie
          </button>
          <button
            :class="['toggle-btn', activeTab === 'tv' ? 'active' : '']"
            @click="setActive('tv')"
          >
            TV
          </button>
...
const likedMovies = computed(() => {
  ...
  // Stelle sicher, dass die Movies geladen sind
  if (!myStore.movies || myStore.movies.length === 0) return [];  

  return myStore.movies.filter(movie => likedMovieIds.includes(movie.id));
});
*/
const likedMovies = computed(() => {
  // myStore.account.favourites should now contain full movie objects fetched from the backend
